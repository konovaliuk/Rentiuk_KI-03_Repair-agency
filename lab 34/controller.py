...
def create_request():
    ...
    if current_user.role == 1:
        new_request = Request(request.form['name'], request.form['model'], request.form['description'],
                              request.form['status'],
                              current_user.id, request.form['price'])
        if request_services.add_request(new_request):
            error = None
        else:
            error = 'Invalid data'
...
def choose_requests():
    ...
    if current_user.role == 2:  # admin
        data = request_services.show_by_status('pending')
        if not data:
            error = 'No requests so far'
    elif current_user.role == 3:  # master
        data = request_services.show_by_status('accepted')
        if not data:
...
def process_choice():
    ...
    if current_user.role == 2:  # admin
        for id in selected_rows:
            request_services.update_status(id, 'accepted')
    if current_user.role == 3:  # master
        for id in selected_rows:
            request_services.update_status(id, 'in-work')
...
def my_requests():
    ...
    error = None
    if current_user.role == 1:
        data = request_services.get_by_id(current_user.id)
        if not data:
            error = 'No requests by you so far'
    if current_user.role == 3:
        requests_id = feedback_serv.get_requests_id(current_user.id)
        for id in requests_id:
            data.append(request_services.get_by_id(id[0]))
        if not data:
